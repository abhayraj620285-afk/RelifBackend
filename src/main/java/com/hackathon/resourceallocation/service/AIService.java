...
//
//    // ─────────────────────────────────────────────────────────────────────────────
//    // RESULT RECORD  — extended with every missing field
//    // ─────────────────────────────────────────────────────────────────────────────
//
...
//
//            // ── Confidence (NEW) ─────────────────────────────────────────────
//            double categoryConfidence, // 0.0–1.0
//            double urgencyConfidence,  // 0.0–1.0
//            double overallConfidence,  // 0.0–1.0
//            String confidenceLabel,    // HIGH | MEDIUM | LOW  (human-readable)
//
//            // ── Explainability (NEW) ─────────────────────────────────────────
//            String reasoning,
//            List<String> matchedKeywords,   // keywords that drove category decision
//            List<String> alternateCategories, // runner-up categories
//
//            // ── Skills (existing + enriched) ────────────────────────────────
//            String suggestedSkills,
//
//            // ── Location (NEW) ───────────────────────────────────────────────
//            String location,           // extracted from text ("unknown" if not found)
//            double locationConfidence, // 0.0–1.0
//
//            // ── People count (NEW) ───────────────────────────────────────────
//            Integer peopleAffected,    // null if not found
//
//            // ── Language (NEW) ───────────────────────────────────────────────
//            String detectedLanguage,   // "en" | "hi" | "mixed"
//            String translatedText,     // non-null when Hindi input was translated
//
...
//                    "khaana","roti","ann","ann","rasad","bhookha"),
...
//    );
//
//    // Hindi → detection signal (Devanagari Unicode range)
//    private static final Pattern DEVANAGARI = Pattern.compile("[\\u0900-\\u097F]+");
//
//    // People-count regex patterns (priority order)
...
//                    Pattern.CASE_INSENSITIVE),
//            Pattern.compile("(\\d[\\d,]{2,})")   // any number ≥ 3 digits as last resort
//    );
//
//    // ─────────────────────────────────────────────────────────────────────────────
//    // PUBLIC ENTRY POINT
...
//
//        // 2. Translate Hindi if needed (best-effort via Gemini)
...
//                log.info("Translated ({}) text for NLP processing", detectedLang);
//            } else {
//                warnings.add("Hindi translation failed — processing original text");
//            }
//        }
//
//        // 3. Local pre-processing (always runs — feeds fallback AND enriches AI result)
...
//        } catch (Exception e) {
//            log.warn("Gemini call failed: {} — using enriched fallback", e.getMessage());
//            warnings.add("Gemini API unavailable — using local analysis");
//        }
//
//        // 5. Merge: Gemini wins on category/urgency/score/reasoning/skills,
//        //           local enriches with location, people, confidence, keywords
...
//                    List.of(),   // keywords filled from local analysis in merge step
//                    alts,
//                    json.path("suggested_skills").asText(""),
//                    locRaw.isEmpty() ? null : locRaw, // null → local extraction wins
//                    locRaw.isEmpty() ? 0.0 : 0.85,
//                    people < 0 ? null : people,
//                    null, null,  // lang / translated filled in buildFinalResult
//                    true, false, warnings
//            );
//        } catch (Exception e) {
...
//
//    // ─────────────────────────────────────────────────────────────────────────────
//    // LOCAL ANALYSIS  (keyword + regex — always runs)
//    // ─────────────────────────────────────────────────────────────────────────────
//
...
//                if (lower.contains(kw)) { score += 1.0; hits.add(kw); }
//            }
...
//        double catConf = totalKw > 0 ? Math.min(topCatScore / Math.max(totalKw * 0.4, 3), 1.0) : 0.0;
//        if (topCat.equals("OTHER")) warnings.add("Could not determine category — no keyword matches");
//
//        // Urgency scoring
...
//                    urgency = level;
//                    int hits = (int) URGENCY_KEYWORDS.get(level).stream().filter(lower::contains).count();
...
//
//        // Location extraction
//        String location = "unknown"; double locConf = 0.0;
//        // 1. Preposition pattern: "in Delhi" / "at Lucknow" / "from Agra"
...
//                }
//            }
//            if (!location.equals("unknown")) break;
//        }
//        // 2. Direct city-name scan fallback
...
//            }
//        }
//        if (location.equals("unknown")) warnings.add("Location not detected in text");
//
...
//        }
//        if (people == null) warnings.add("No people count detected in text");
//
...
//
//    // ─────────────────────────────────────────────────────────────────────────────
//    // MERGE: Gemini result + local enrichment
//    // ─────────────────────────────────────────────────────────────────────────────
//
...
//
//        // Confidence: prefer Gemini values; average with local for calibration
...
//
//        // Location: Gemini wins if it found something; else use local
//        String location   = (gemini != null && gemini.location() != null && !gemini.location().isBlank())
//                ? gemini.location() : local.location();
//        double locConf    = (gemini != null && gemini.location() != null && !gemini.location().isBlank())
...
//        // Check for Hindi transliterations as secondary signal
...
//
//    // ─────────────────────────────────────────────────────────────────────────────
//    // HINDI TRANSLATION via Gemini (best-effort, returns null on failure)
//    // ─────────────────────────────────────────────────────────────────────────────
//
...
public class AIService {
    ...
    public AIAnalysisResult analyzeNeed(String title, String description) {
        ...
        try {
            ...
            log.warn("Gemini call failed: {} — using enriched keyword fallback", e.getMessage());
            warnings.add("Gemini API unavailable — using local keyword analysis");
        }
